...
namespace OSCManager.Core.Services
{
    ...
    {
        public const string CacheKey = nameof(SourceRepository);
        ...
        private async ValueTask<IDictionary<string, SourceRepository>> GetDictionaryAsync(CancellationToken cancellationToken)
        {
            return await _memoryCache.GetOrCreate(CacheKey, async entry =>
            {
                // TODO
                return new Dictionary<string, SourceRepository>();
